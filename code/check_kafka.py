...
################################################################################
class BrokerRequestHandler(SimpleHTTPServer.SimpleHTTPRequestHandler):
    ...
    def _set_headers(self):
        command1 = "ps -ef | grep -E 'java.*kafka.*server\.properties' | grep -v grep | wc -l"
        ...
        if self.check_output == "0":
            self.my_util.log.error("check_kafka: [500] {}".format(self.check_message))
            self.send_response(500)
        else:
            self.my_util.log.info("check_kafka: [200] {}".format(self.check_message))
